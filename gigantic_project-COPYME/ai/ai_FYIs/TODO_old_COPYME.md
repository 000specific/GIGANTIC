...
existing patterns from `subprojects/orthogroups/BLOCK_orthohmm_GIGANTIC/` or
`subprojects/gene_sizes/BLOCK_analyze_gene_sizes/` parse cleanly under
NextFlow 26.
...
The fully-debugged reference is in this COPYME tree at:
```
...
### Action items (post-shutdown 2026-05-06)
...
- [ ] **Audit existing BLOCKs** for NextFlow 26 compatibility:
  ...
  - [ ] Any other workflow with `nextflow.config` containing `import` / `def` / `$slurm` / `onComplete`
...
- [ ] **Document the working pattern** in this project's AI_GUIDE hierarchy
...
`subprojects/hotspots/BLOCK_self_blast/workflow-COPYME-self_blast/ai/nextflow.config`
