#!/bin/bash
# AI: Claude Code | Opus 4.7 | 2026 May 29 | Purpose: One-script entry point for the Gene Coordinates Extractor toolkit
# Human: Eric Edsinger
# =============================================================================
# RUN-workflow.sh - Gene Coordinates Extractor toolkit
# =============================================================================
...
#   2. Auto-creates the conda env `aiG-research_ai-gene_coordinates_extractor`
...
case "$( basename "${SCRIPT_DIR}" )" in
    *COPYME*)
        ...
        echo "  cp -r toolkit-COPYME-gene_coordinates_extractor toolkit-RUN_1-gene_coordinates_extractor"
        echo "  cd toolkit-RUN_1-gene_coordinates_extractor"
...
echo '========================================================================'
echo 'Gene Coordinates Extractor Toolkit'
echo '========================================================================'
echo "Started:           $( date )"
...
if [ "${EXECUTION_MODE}" = "slurm" ] && [ -z "${SLURM_JOB_ID:-}" ]; then
    ...
    sbatch \
        ...
        --job-name="gene_coordinates_extractor" \
        --output="slurm_logs/gene_coordinates_extractor-%j.log" \
...
# -----------------------------------------------------------------------------
ENV_NAME="aiG-research_ai-gene_coordinates_extractor"
...
# Species whitelist: accept either YAML list or comma-separated string.
sw = inputs.get( "species_whitelist", "" ) or ""
if isinstance( sw, list ):
    sw_str = ",".join( str( x ).strip() for x in sw if str( x ).strip() )
else:
    sw_str = str( sw ).strip()

flat = {
    "annotations_dir":                 resolve( inputs.get( "annotations_dir" ) ),
    "hotspots_gene_coordinates_dir":   resolve( inputs.get( "hotspots_gene_coordinates_dir" ) ),
    "species_whitelist":               sw_str,
...
if [ "${EXIT_CODE}" -eq 0 ]; then
    echo 'SUCCESS! Gene Coordinates Extractor completed.'
    echo ""
    echo "Outputs:"
