#!/usr/bin/env bash
# AI: Claude Code | Opus 4 | 2026 February 12 | Purpose: Download Schizocardium californicum proteome (MATEdb2 fallback to NCBI GCA_019649055.1)
# Human: Eric Edsinger

# Strategy: Try MATEdb2 Zenodo first, then fall back to NCBI datasets
# Note: MATEdb2 hemichordata has SKOW1 (Saccoglossus kowalevskii) but NOT Schizocardium
# NCBI fallback: GCA_019649055.1
...
NCBI_ACCESSION="GCA_019649055.1"
...
if [ -s "${TEMP_DIR}/linksforMATEdb2.txt" ]; then
  CODE=$( grep -i "schizocardium\|californicum\|scal" "${TEMP_DIR}/linksforMATEdb2.txt" 2>/dev/null | head -1 | awk '{print $2}' || true )
fi
if [ -z "${CODE}" ]; then
  CODE="SCAL1"
fi
...
if [ "${PROTEIN_FOUND}" = false ]; then
  ...
  echo "Trying NCBI: ${NCBI_ACCESSION}" >> "${LOG_FILE}"

  if [ -x "${NCBI_DATASETS}" ]; then
    # Download genome package with protein and genome
    ...
    if [ -s "${TEMP_DIR}/ncbi_dataset.zip" ]; then
      ...
      if [ -n "${NCBI_PROTEIN}" ] && [ -s "${NCBI_PROTEIN}" ]; then
        ...
        echo "Protein source: NCBI datasets ${NCBI_ACCESSION}" >> "${LOG_FILE}"
        echo "Protein file: ${NCBI_PROTEIN}" >> "${LOG_FILE}"
        echo "  Proteome downloaded from NCBI (${NCBI_ACCESSION})"
        PROTEIN_FOUND=true
      else
