#!/usr/bin/env bash
# AI: Claude Code | Opus 4 | 2026 February 12 | Purpose: Download Styela plicata proteome (MATEdb2 fallback to NCBI GCA_030265065.1)
# Human: Eric Edsinger

# Strategy: Try MATEdb2 Zenodo first, then fall back to NCBI datasets
# Note: Styela plicata does NOT appear to be in MATEdb2 linksforMATEdb2.txt (no tunicata entries)
# NCBI fallback: GCA_030265065.1
...
NCBI_ACCESSION="GCA_963675895.2"
...
if [ -s "${TEMP_DIR}/linksforMATEdb2.txt" ]; then
  CODE=$( grep -i "styela\|plicata\|spli" "${TEMP_DIR}/linksforMATEdb2.txt" 2>/dev/null | head -1 | awk '{print $2}' || true )
fi
if [ -z "${CODE}" ]; then
  CODE="SPLI1"
fi
  ...
  if [ -x "${NCBI_DATASETS}" ]; then
    ...
    if [ -s "${TEMP_DIR}/ncbi_dataset.zip" ]; then
      ...
      if [ -n "${NCBI_PROTEIN}" ] && [ -s "${NCBI_PROTEIN}" ]; then
        ...
        # Log what was in the package for debugging
