#!/usr/bin/env bash
# AI: Claude Code | Opus 4 | 2026 February 12 | Purpose: Download Creolimax fragrantissima from Dryad (doi:10.5061/dryad.dncjsxm47, Schultz et al. 2023)
# Human: Eric Edsinger
...
SPECIES="Creolimax_fragrantissima"
...
# Find species-specific files (Creolimax, creolimax, Cfrag, cfrag, fragrantissima)
...
GENOME_FILE=$( find "${SEARCH_DIR}" -type f \( -path "*Creolimax*" -o -path "*creolimax*" -o -path "*Cfrag*" -o -path "*cfrag*" -o -path "*fragrantissima*" \) \( -name "*gDNA*" -o -name "*assembly*" -o -name "*genome*" \) \( -name "*.fasta" -o -name "*.fa" -o -name "*.fna" \) 2>/dev/null | head -1 )
if [ -z "${GENOME_FILE}" ]; then
  GENOME_FILE=$( find "${SEARCH_DIR}" -type f \( -path "*Creolimax*" -o -path "*creolimax*" -o -path "*Cfrag*" -o -path "*cfrag*" -o -path "*fragrantissima*" \) \( -name "*.fasta" -o -name "*.fa" -o -name "*.fna" \) -exec wc -c {} \; 2>/dev/null | sort -rn | head -1 | awk '{print $2}' )
fi
if [ -z "${GENOME_FILE}" ]; then
...
ANNOT_FILE=$( find "${SEARCH_DIR}" -type f \( -path "*Creolimax*" -o -path "*creolimax*" -o -path "*Cfrag*" -o -path "*cfrag*" -o -path "*fragrantissima*" \) \( -name "*.gff" -o -name "*.gff3" -o -name "*.gtf" \) 2>/dev/null | head -1 )
...
PROTEIN_FILE=$( find "${SEARCH_DIR}" -type f \( -path "*Creolimax*" -o -path "*creolimax*" -o -path "*Cfrag*" -o -path "*cfrag*" -o -path "*fragrantissima*" \) \( -name "*pep*" -o -name "*protein*" -o -name "*proteome*" \) \( -name "*.fasta" -o -name "*.fa" -o -name "*.faa" -o -name "*.pep" \) 2>/dev/null | grep -v -i "cds" | head -1 || true )
if [ -z "${PROTEIN_FILE}" ]; then
  PROTEIN_FILE=$( find "${SEARCH_DIR}" -type f \( -path "*Creolimax*" -o -path "*creolimax*" -o -path "*Cfrag*" -o -path "*cfrag*" -o -path "*fragrantissima*" \) \( -name "*pep*" -o -name "*protein*" -o -name "*proteome*" -o -name "*.faa" -o -name "*.pep" \) 2>/dev/null | grep -v -i "cds" | head -1 || true )
fi
if [ -z "${PROTEIN_FILE}" ]; then
