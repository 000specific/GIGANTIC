...
params.source_manifest = "INPUT_user/source_identifiers_manifest.tsv"
 ...
 *
 * Renames intermediate files (Genus_species-repository_genomes-T1_proteome.aa) to
 * standardized GIGANTIC convention (Genus_species-genome_SOURCE_ID-downloaded_DATE.ext)
...
process reformat_genomesdb_input {
    ...
    python3 ${projectDir}/scripts/005_ai-python-reformat_genomesdb_input.py \\
        --source-manifest "${projectDir}/../${params.source_manifest}" \\
...
workflow.onComplete {
    ...
    if (workflow.success) {
        ...
        println "  5-output/  GIGANTIC convention (genomes/, gene_annotations/, T1_proteomes/, maps/)"
        println ""
        println "T1 proteomes: ${params.output_dir}/3-output/T1_proteomes/"
        println "GIGANTIC files: ${params.output_dir}/5-output/"
        println "Symlinks for downstream: output_to_input/"
    }
    println "========================================================================"
}
