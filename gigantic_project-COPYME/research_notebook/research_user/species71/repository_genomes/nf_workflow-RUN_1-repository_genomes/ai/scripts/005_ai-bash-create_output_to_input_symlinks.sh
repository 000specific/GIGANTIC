...
if [ "${AA_FILE_COUNT}" -eq 0 ]; then
    echo "WARNING: No .aa files found in ${ENUMERATE_DIRECTORY}"
    echo "This may be expected if no species have been downloaded yet."
    mkdir -p 4-output
    echo -e "symlink_filename\trelative_target_path\tabsolute_source_path" > 4-output/symlinks_manifest.tsv
    echo "Created empty manifest."
    exit 0
fi
...
# Note: Symlinks may show as BROKEN here because publishDir has not yet copied
