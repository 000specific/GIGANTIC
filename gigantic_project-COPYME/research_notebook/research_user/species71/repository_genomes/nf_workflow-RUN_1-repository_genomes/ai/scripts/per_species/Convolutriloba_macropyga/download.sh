#!/usr/bin/env bash
# AI: Claude Code | Opus 4 | 2026 February 12 | Purpose: Download Convolutriloba macropyga proteome (MATEdb2 attempt, likely needs user input)
# Human: Eric Edsinger

# Strategy: Try MATEdb2 Zenodo first, but this species is likely NOT in MATEdb2
# MATEdb2 acoela only has HMIA1, SYRO2 - NOT Convolutriloba
...
SPECIES="Convolutriloba_macropyga"
...

{
  ...
  echo "Neither Convolutriloba nor macropyga found in linksforMATEdb2.txt"
  echo ""
} > "${LOG_FILE}"
...
if [ -s "${TEMP_DIR}/linksforMATEdb2.txt" ]; then
  CODE=$( grep -i "convolutriloba\|macropyga\|cmac" "${TEMP_DIR}/linksforMATEdb2.txt" 2>/dev/null | head -1 | awk '{print $2}' || true )
fi
if [ -z "${CODE}" ]; then
  CODE="CMAC1"
fi
...
if [ "${PROTEIN_FOUND}" = true ] && [ -s "${OUTPUT_DIRECTORY}/protein.faa" ]; then
  ...
  echo "" >> "${LOG_FILE}"
  echo "This species (Convolutriloba macropyga) is an acoela that may only be" >> "${LOG_FILE}"
