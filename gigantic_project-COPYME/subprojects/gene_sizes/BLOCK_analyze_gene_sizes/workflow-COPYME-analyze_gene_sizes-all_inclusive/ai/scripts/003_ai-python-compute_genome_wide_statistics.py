...
def main():
    ...
    with open( gene_metrics_file, 'r' ) as input_file:
        ...
        for line in input_file:
            ...
            # 002 output column order:
            # 0: Source_Gene_ID, 1: GIGANTIC_Identifier, 2: Seqid, 3: Start, 4: End,
            # 5: Strand, 6: Gene_Size, 7: Transcript_Size, 8: CDS_Size, 9: Protein_Size,
            # 10: Exon_Count, 11: Intron_Total_Size, 12: UTR_Total_Size,
            # 13: Exon_Sizes_Ordered, 14: Intron_Sizes_Ordered
            genes.append( {
                ...
                'transcript_size': int( parts[ 7 ] ),
                'cds_size': int( parts[ 8 ] ),
                'protein_size': int( parts[ 9 ] ),
                'exon_count': int( parts[ 10 ] ),
                'intron_total_size': int( parts[ 11 ] ),
                'utr_total_size': int( parts[ 12 ] ),
                'exon_sizes_ordered': parts[ 13 ],
                'intron_sizes_ordered': parts[ 14 ]
            } )
    ...
    transcript_sizes = [ gene[ 'transcript_size' ] for gene in genes ]
    ...
    intron_total_sizes = [ gene[ 'intron_total_size' ] for gene in genes ]
    utr_total_sizes = [ gene[ 'utr_total_size' ] for gene in genes ]

    # Compute quantile ranks
    logger.info( 'Computing quantile ranks...' )
    gene_size_ranks = compute_quantile_rank( gene_sizes )
    transcript_size_ranks = compute_quantile_rank( transcript_sizes )
    ...
    intron_total_size_ranks = compute_quantile_rank( intron_total_sizes )
    ...
    with open( ranked_output_file, 'w' ) as output_file:
        ...
        output = header_line + '\t' + \
                 ...
                 'Transcript_Size_Rank (quantile rank 0 to 1 within genome for transcript size)' + '\t' + \
                 ...
                 'Intron_Total_Size_Rank (quantile rank 0 to 1 within genome for total intronic size)' + '\n'
    ...
    metrics_to_summarize = {
        ...
        'Transcript_Size': transcript_sizes,
        'CDS_Size': cds_sizes,
        'Protein_Size': protein_sizes,
        'Exon_Count': exon_counts,
        'Intron_Total_Size': intron_total_sizes,
        'UTR_Total_Size': utr_total_sizes,
    }
