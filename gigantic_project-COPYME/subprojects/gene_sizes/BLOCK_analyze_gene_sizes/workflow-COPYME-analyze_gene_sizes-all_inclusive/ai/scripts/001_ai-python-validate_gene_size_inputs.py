...
"""
GIGANTIC gene_sizes - Script 001: Validate Gene Size Inputs (Tier 1: all_inclusive)

Purpose:
    Validates user-provided 15-column Tier 1 gene structure TSV files in
    INPUT_user/ against the GIGANTIC species set. Determines which species
    can be processed (have valid gene structure data) and which must be
    skipped (no data or incomplete data). Produces a species processing
    status report.
    ...
    This is the all_inclusive (Tier 1) variant — see the parallel
    gene_vs_protein (Tier 2) workflow for the broader-coverage tier with
    fewer required fields.

Inputs:
    --input-dir: Path to INPUT_user/ containing per-species 15-col Tier 1 TSVs
    --gigantic-species-list: Path to GIGANTIC species list (all species in set)
    --output-dir: Output directory

    User-provided TSV format — 15 columns; one file per qualifying species,
    named Genus_species-gene_coordinates_all_inclusive.tsv:
...
def validate_gene_structure_file( gene_structure_file: Path, genus_species: str, logger: logging.Logger ) -> dict:
    ...
    missing_exon_count = 0
    missing_cds_count = 0
    header_found = False

    # 15-column Tier 1 (all_inclusive) TSV produced by the gene_coordinates extractors:
    # Source_Gene_ID  Seqid  Gene_Start  Gene_End  Strand  Gene_Size
    # Exon_Intervals  Transcript_Size  CDS_Intervals  CDS_Size  Protein_Size
    # UTR_5prime_Intervals  UTR_5prime_Size  UTR_3prime_Intervals  UTR_3prime_Size
    # parts[6] = Exon_Intervals; parts[8] = CDS_Intervals (used for validation below).
    with open( gene_structure_file, 'r' ) as input_file:
        for line in input_file:
            # Use rstrip('\n') NOT strip() — bare strip() removes trailing tabs
            # because '\t' is whitespace, which silently drops rows whose final
            # column is empty.
            line = line.rstrip( '\n' )
            ...
            # 15-col all_inclusive TSV — see header in 002.
            if len( parts ) < 15:
            ...
            # Validate Exon_Intervals (required for transcript_size and intron metrics)
            if not exon_intervals_string:
                missing_exon_count += 1
                continue

            # Validate CDS_Intervals (required for cds_size and protein_size)
...
def main():
    ...
    for genus_species in gigantic_species:
        # Look for Tier 1 user-provided file: Genus_species-gene_coordinates_all_inclusive.tsv
        gene_structure_file = input_dir / f'{genus_species}-gene_coordinates_all_inclusive.tsv'
    ...
    if processed_count == 0:
        ...
        logger.error( '  Provide per-species TSV files in INPUT_user/ (Genus_species-gene_coordinates_all_inclusive.tsv, 15-col Tier 1 layout)' )
