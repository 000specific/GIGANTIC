...
"""
GIGANTIC gene_sizes - Script 001: Validate Gene Size Inputs (Tier 2: gene_vs_protein)

Purpose:
    Validates user-provided 9-column Tier 2 gene structure TSV files in
    ...
    This is the gene_vs_protein (Tier 2) variant — see the parallel
    all_inclusive (Tier 1) workflow for the methodology-strict tier with
    the full 15-column schema and 7 metrics. Tier 1 species are also
    written as Tier 2 (superset overlap), so this tier covers the broadest
    species set on the universally-comparable gene + CDS subset.

Inputs:
    --input-dir: Path to INPUT_user/ containing per-species 9-col Tier 2 TSVs
    --gigantic-species-list: Path to GIGANTIC species list (all species in set)
    --output-dir: Output directory

    User-provided TSV format — 9 columns; one file per qualifying species,
    named Genus_species-gene_coordinates_gene_vs_protein.tsv:
...
def validate_gene_structure_file( gene_structure_file: Path, genus_species: str, logger: logging.Logger ) -> dict:
    ...
    # 9-column gene_vs_protein TSV produced by the gene_coordinates extractors:
...
def main():
    ...
    for genus_species in gigantic_species:
        # Look for Tier 2 user-provided file: Genus_species-gene_coordinates_gene_vs_protein.tsv
        gene_structure_file = input_dir / f'{genus_species}-gene_coordinates_gene_vs_protein.tsv'
    ...
    if processed_count == 0:
        ...
        logger.error( '  Provide per-species TSV files in INPUT_user/ (Genus_species-gene_coordinates_gene_vs_protein.tsv, 9-col Tier 2 layout)' )
