...
"""
GIGANTIC gene_sizes - Script 002: Extract Gene Metrics (Tier 2: gene_vs_protein)

Purpose:
    Reads user-provided 9-column Tier 2 gene structure data (gene + CDS only)
    ...
    This is the broad-coverage tier (Tier 2) — see the parallel all_inclusive
    workflow for the methodology-strict 15-col / 7-metric tier.

    Tier 2 deliberately omits transcript / intron / UTR / exon-count metrics
    because the underlying species' annotations don't reliably distinguish
    UTR from non-UTR exonic content. Including those metrics here would be
    methodologically inconsistent with Tier 1.

Inputs:
    --gene-structure-file: Path to 9-col Tier 2 TSV for one species
    ...
    User-provided TSV format (9 columns):
        Source_Gene_ID  Seqid  Gene_Start  Gene_End  Strand  Gene_Size
        CDS_Intervals  CDS_Size  Protein_Size

Outputs:
    2-output/2_ai-gene_metrics-{Genus_species}.tsv - 11-col per-gene metrics with GIGANTIC IDs
    2-output/2_ai-log-extract_gene_metrics-{Genus_species}.log - Execution log

Metrics computed per gene (Tier 2 — 4 metrics):
    ...
    CDS_Segment_Count    - Number of merged CDS intervals (coarse proxy for exon count)
    CDS_Sizes_Ordered    - Individual CDS interval sizes in 5'-to-3' transcript order

The script re-derives sizes from the intervals (rather than trusting the
...
def main():
    ...
    # 9-col Tier 2 (gene_vs_protein) input from the gene_coordinates extractor:
    # Source_Gene_ID  Seqid  Gene_Start  Gene_End  Strand  Gene_Size  CDS_Intervals  CDS_Size  Protein_Size
    # ENSG00000139618  chr13  32315474  32400266  +  84793  32316422-32316527,32319077-32319325,...  10256  3418
    header_found = False
    with open( gene_structure_file, 'r' ) as input_file:
        for line in input_file:
            ...
            # 9-column gene_vs_protein TSV format from gene_coordinates extractors:
            #  0: Source_Gene_ID, 1: Seqid, 2: Gene_Start, 3: Gene_End, 4: Strand,
            #  5: Gene_Size, 6: CDS_Intervals, 7: CDS_Size, 8: Protein_Size
            cds_intervals_string = parts[ 6 ].strip()
            ...
            # Three biologically meaningful metrics for tier-2 (gene_vs_protein) species:
            #   gene_size    = genomic span (gene feature end - start + 1)
            #   cds_size     = sum of CDS lengths (the coding portion only)
            #   protein_size = cds_size / 3 (estimated amino acids)
            # transcript_size, intron_total_size, utr_total_size are NOT computed here:
            # the source annotation does not reliably distinguish UTR from non-UTR exonic
            # content, so a transcript-level metric would be methodologically inconsistent
            # with tier-1 species. (Tier-1 species are processed by the all_inclusive
            # workflow, which produces all 7 metrics.)
            gene_size = gene_end - gene_start + 1
            cds_size = sum( end - start + 1 for start, end in merged_cds )
            protein_size = cds_size // 3
            cds_segment_count = len( merged_cds )

            cds_sizes = [ end - start + 1 for start, end in merged_cds ]
            if strand == '-':
                cds_sizes = cds_sizes[ ::-1 ]
            cds_sizes_string = ','.join( str( size ) for size in cds_sizes )
            ...
            gene_metrics.append( {
                ...
                'cds_segment_count': cds_segment_count,
                'cds_sizes_ordered': cds_sizes_string,
            } )
    ...
    # Write output with biologically correct metric names (3 metrics for tier-2 species).
    ...
    with open( output_file_path, 'w' ) as output_file:
        output = 'Source_Gene_ID (source gene identifier matching g_ field in GIGANTIC headers)' + '\t' + \
                 ...
                 'CDS_Segment_Count (number of merged CDS intervals)' + '\t' + \
                 'CDS_Sizes_Ordered (comma delimited individual CDS interval sizes in bp ordered 5-prime to 3-prime along transcript)' + '\n'
