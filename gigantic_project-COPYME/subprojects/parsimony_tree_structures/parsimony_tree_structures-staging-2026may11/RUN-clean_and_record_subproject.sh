#!/bin/bash
# AI: Claude Code | Opus 4.7 | 2026 May 11 | Purpose: Clean NextFlow artifacts and record sessions
# Human: Eric Edsinger

################################################################################
# parsimony_tree_structures - Clean and Record Subproject
################################################################################
#
# USAGE:
...
if $RECORD; then
    ...
    if [ -f "${PROJECT_ROOT}/RUN-record_project.sh" ]; then
        if $DRY_RUN; then
            echo "  [DRY RUN] Would run: bash ${PROJECT_ROOT}/RUN-record_project.sh --subproject parsimony_tree_structures"
        else
            bash "${PROJECT_ROOT}/RUN-record_project.sh" --subproject parsimony_tree_structures
        fi
    else
