...
**For AI Assistants**: Read `../../AI_GUIDE-project.md` first for GIGANTIC overview, directory structure, and general patterns. This guide covers trees_gene_groups-specific concepts and the three-step architecture.
...
| trees_gene_groups concepts, three-step structure | This file |
...
**Purpose**: Build phylogenetic trees for gene groups (orthogroup-derived sets) across GIGANTIC species.

**Relationship to trees_gene_families**: This subproject uses the **identical three-step pipeline** as trees_gene_families, but operates on gene groups rather than individually curated gene families. Gene groups are typically larger sets of related genes identified by orthogroup tools (OrthoFinder, OrthoHMM, Broccoli) rather than hand-curated reference sequences.
...
**Critical**: Run genomesDB subproject FIRST - trees_gene_groups depends on genomesDB for BLAST databases and proteome files.
...
### Gene Groups vs. Gene Families

| Aspect | trees_gene_families | trees_gene_groups |
|--------|--------------------|--------------------|
| Input source | Hand-curated reference sequences | Orthogroup-derived gene sets |
| Scale | Typically a few families at a time | Potentially many groups in batch |
| RGS curation | Manual selection from UniProt/NCBI | Derived from orthogroup analysis |
| Pipeline | Identical three-step RBH/RBF | Identical three-step RBH/RBF |

### One Gene Group Per Run

Each workflow copy processes **one gene group at a time**:

```bash
# Example: Processing an orthogroup-derived gene group
cp -r workflow-COPYME-rbh_rbf_homologs workflow-RUN_01-rbh_rbf_homologs
cd workflow-RUN_01-rbh_rbf_homologs
# Edit config with gene group name, then run
```

To process multiple gene groups, create multiple RUN copies.
...
|----------|-------------------------------|
| `trees_gene_groups/` | `../../` |
| `trees_gene_groups/STEP_N-*/` | `../../../` |
| `trees_gene_groups/STEP_N-*/workflow-COPYME-*/` | `../../../../` |
| `trees_gene_groups/STEP_N-*/workflow-COPYME-*/ai/` | `../../../../../` |

---

## Conda Environment

**Environment name**: `ai_gigantic_trees_gene_groups`
**Definition file**: `../../conda_environments/ai_gigantic_trees_gene_groups.yml`
...
All trees_gene_groups AI sessions save to:
```
research_notebook/research_ai/subproject-trees_gene_groups/
├── logs/
└── sessions/
```
...
| Before STEP_1 | "What gene group? Do you have a curated RGS FASTA file?" |
...
| Multiple gene groups | "How many gene groups? You'll need one workflow copy per group." |
...
3. **Additional gene groups** - Create more RUN copies for other groups
