...
This script extracts the species (second field) and uses it to match against
...
def parse_rgs_header( header: str ) -> Tuple[bool, str, str, str]:
    ...
    # HGNC index positions: rgs_FAMILY-SPECIES-GENE-SOURCE-IDENTIFIER
    ...
    for blast_report in blast_reports:
        ...
        with open( blast_report, 'r' ) as input_file:
            for line in input_file:
                ...
                # HGNC RGS header format (5 fields): >rgs_{family}-{species}-{gene_symbol}-{source}-{identifier}
                # Extract species from second field (index 1)
