...
# After each per-gene-group nextflow run succeeds, symlink the AGS file from
# OUTPUT_pipeline/16-output/ → output_to_input/<instance>/STEP_1-homolog_discovery/gene_group-<gg>/
# so downstream subprojects (STEP_2 here, plus other GIGANTIC subprojects) can
# consume the AGS without knowing the per-gene-group RUN_01 layout.
# STEP_1 is single-track (no PARENT_RUN_NAME); single output target per gene group.
# =============================================================================
...
if [ -z "$( cd "${STEP1_DIR}/../../output_to_input" 2>/dev/null && pwd )" ]; then
    # output_to_input dir doesn't exist yet; resolve via realpath of the would-be path
...
oti_publish_step1() {
    local gg="$1"
    local dest="$2"
    local target="${OTI_INSTANCE_DIR}/STEP_1-homolog_discovery/gene_group-${gg}"
    # Explicit destination filename (not 'TARGET/' with trailing slash) — the
    # trailing-slash GNU-coreutils-behavior is unreliable on the /blue network FS
    # and fails with 'cannot overwrite directory'.
    # Trailing 'true' guarantees exit 0 even if the glob doesn't match (avoids
    # a false-positive FAILED report at the wrap layer when STEP_1's AGS file
    # has an unexpected name or is genuinely missing).
    echo "mkdir -p '${target}' && for f in '${dest}/OUTPUT_pipeline/16-output/'16_ai-ags-*.aa; do [ -f \"\$f\" ] && ln -sf \"\$f\" \"${target}/\$( basename \"\$f\" )\"; done; true"
}
...
case "${EXECUTION_MODE}" in
    local)
        ...
        for gg in "${SMALL_GG[@]}" "${LARGE_GG[@]}"; do
            ...
            echo "------------------------------------------------------------"
            ( cd "${DEST}" && nextflow run ai/main.nf ${RESUME_FLAG} -c ai/nextflow.config -params-file .params.json && eval "$(oti_publish_step1 "${gg}" "${DEST}")" ) \
                || echo "  FAILED: ${gg}"
        done
        ;;
    ...
    slurm-standard)
        ...
        for tier in small large; do
            ...
            for gg in "${arr[@]}"; do
                DEST="${STEP1_DIR}/gene_group-${gg}/workflow-RUN_01-rbh_rbf_homologs"
                wrap="module load conda 2>/dev/null || true; conda activate ${ENV_NAME}; ( $(nextflow_run_cmd "${DEST}") && $(oti_publish_step1 "${gg}" "${DEST}") ) || echo \"FAILED: ${gg}\""
                sbatch \
    ...
    slurm-burst)
        ...
        for tier in small large; do
            ...
            while [ "$i" -lt "$n" ]; do
                ...
                # each followed by an OTI publish step on success.
                ...
                while [ "$j" -lt "$end" ]; do
                    ...
                    runner+="( $(nextflow_run_cmd "${DEST}") && $(oti_publish_step1 "${gg}" "${DEST}") ) || echo \"FAILED: ${gg}\"; "
                    j=$((j + 1))
                done
