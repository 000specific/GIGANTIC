...
GIGANTIC trees_gene_groups STEP_3 workflow template. Renders phylogenetic trees (produced by STEP_2) as PDF + SVG using toytree.
...
Symlinked into `../../../output_to_input/gene_groups-hugo_hgnc/STEP_3-tree_visualization/gene_group-<gene_family>/`.

## Usage

```bash
# 1. Copy the template (one workflow instance per gene group)
cp -r workflow-COPYME-tree_visualization workflow-RUN_1-tree_visualization
cd workflow-RUN_1-tree_visualization/

# 2. Configure (set gene group name to match STEP_2 run)
...
1. Reads gene group name from `START_HERE-user_config.yaml`
2. Validates that STEP_2 output exists for this gene group
3. Activates the `aiG-trees_gene_groups-visualization` conda env
...
6. Creates symlinks in `output_to_input/gene_groups-hugo_hgnc/STEP_3-tree_visualization/gene_group-<gene_family>/`
