...
"""
Visualize phylogenetic trees from Newick format files.

This script takes FastTree and IQ-TREE Newick files and generates both SVG and PDF
visualizations of the phylogenetic trees. It uses ETE3 toolkit for tree rendering.

Usage:
    python3 008_ai-python-visualize_phylogenetic_trees-computer_vision_friendly.py [options]
    
Options:
    --bootstraps             Show bootstrap values (default: no)
    --ultrafast_bootstraps   Show ultrafast bootstrap values (default: no)
    --branch_lengths         Show branch lengths (default: no)
    ...
    --color                  Enable colored bootstrap values (default: no, auto-enabled if bootstraps shown)
    --node_identifiers       Show clade ID labels for all nodes (CNNN format) (default: no)

Examples:
    # Basic visualization (no annotations, no scale bar)
    python3 008_ai-python-visualize_phylogenetic_trees-computer_vision_friendly.py
    
    # Show colored bootstrap values (FastTree: bootstrap, IQ-TREE: SH-aLRT)
    python3 008_ai-python-visualize_phylogenetic_trees-computer_vision_friendly.py --bootstraps
    
    # Show both SH-aLRT and UFBoot for IQ-TREE (top and bottom of branches)
    python3 008_ai-python-visualize_phylogenetic_trees-computer_vision_friendly.py --bootstraps --ultrafast_bootstraps
    
    # Show clade IDs for identifying rooting points and species/clades
    python3 008_ai-python-visualize_phylogenetic_trees-computer_vision_friendly.py --node_identifiers
    
    # Show everything: bootstraps, UFBoot, and clade IDs (for rooting and clade identification)
    python3 008_ai-python-visualize_phylogenetic_trees-computer_vision_friendly.py --bootstraps --ultrafast_bootstraps --node_identifiers
    
    # Show branch lengths and bootstraps with scale bar and custom threshold
    python3 008_ai-python-visualize_phylogenetic_trees-computer_vision_friendly.py --bootstraps --branch_lengths --bootstrap_threshold 0.9

Notes:
    - FastTree trees show only bootstrap support values
    - IQ-TREE trees show SH-aLRT (top) and/or UFBoot (bottom) support values
    - Scale bar only appears when --branch_lengths is specified
    - Clade IDs (CNNN format) shown as medium-light grey labels on right side of all nodes
    ...
    - Node symbol size is 60x branch line thickness (default: 120 pixels)
...
Output Files:
    - output/8-AGS-species67_T1-species37-innexin_pannexin-fasttree-computer_vision_friendly.svg
    - output/8-AGS-species67_T1-species37-innexin_pannexin-fasttree-computer_vision_friendly.pdf
    - output/8-AGS-species67_T1-species37-innexin_pannexin-iqtree-computer_vision_friendly.svg
    - output/8-AGS-species67_T1-species37-innexin_pannexin-iqtree-computer_vision_friendly.pdf
...
class VisualizationConfig:
    ...
    def __init__(
        ...
        show_ultrafast_bootstraps: bool = False,
        ...
        self.color_clade_id = "#000000"  # Black for clade IDs
        self.color_branch_length = "#5AC8FA"  # Teal (Apple)
        
        # Node symbol settings
        self.branch_line_width = 4  # Base branch line width (doubled)
        self.node_symbol_size = 100  # 100 pixels for computer vision
        self.leaf_symbol = "circle"  # Tips/leaves/species
        self.internal_symbol = "square"  # Internal nodes/ancestral clades
        
        # Font sizes for different text elements (all 20 for computer vision)
        self.font_size_leaf_names = 20  # Tip/leaf names
        self.font_size_small = 20  # Bootstraps, branch lengths, clade identifiers
        
        # Spacing for leaf names and clade identifiers
        self.leaf_name_margin = 15  # pixels of space between branch end and tip name
        self.text_padding = 5  # pixels of padding on all sides for OCR-friendly text separation
...
def parse_arguments() -> VisualizationConfig:
    ...
    parser.add_argument(
        '--bootstraps',
        action='store_true',
        help='Show bootstrap support values (default: no)'
    )
    
    parser.add_argument(
        '--ultrafast_bootstraps',
        action='store_true',
        help='Show ultrafast bootstrap support values for IQ-TREE (default: no)'
    )
    ...
    parser.add_argument(
        '--color',
        action='store_true',
        help='Enable colored bootstrap values (default: no, auto-enabled if bootstraps shown)'
    )
    
    parser.add_argument(
        '--node_identifiers',
        action='store_true',
        help='Show clade ID labels (CNNN format) for all nodes - species and ancestral clades (default: no)'
    )
    
    arguments = parser.parse_args()
    
    return VisualizationConfig(
        show_bootstraps=arguments.bootstraps,
        show_ultrafast_bootstraps=arguments.ultrafast_bootstraps,
        ...
        color_bootstraps=arguments.color,
        show_node_identifiers=arguments.node_identifiers
    )
...
def main():
    ...
    if input_fasttree_path:
        ...
        # Create output prefix by replacing the leading number with "8"
        fasttree_parts = fasttree_base.split( '-', 1 )
        if len( fasttree_parts ) == 2:
            output_fasttree_prefix = output_directory / f"8-{fasttree_parts[ 1 ]}-fasttree-computer_vision_friendly"
        else:
            output_fasttree_prefix = output_directory / f"8-{fasttree_base}-fasttree-computer_vision_friendly"
    else:
    ...
    if input_iqtree_path:
        ...
        if len( iqtree_parts ) == 2:
            output_iqtree_prefix = output_directory / f"8-{iqtree_parts[ 1 ]}-iqtree-computer_vision_friendly"
        else:
            output_iqtree_prefix = output_directory / f"8-{iqtree_base}-iqtree-computer_vision_friendly"
    else:
