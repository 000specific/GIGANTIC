#!/bin/bash
# AI: Claude Code | Opus 4.6 | 2026 March 10 | Purpose: Run SignalP Nextflow pipeline
# Human: Eric Edsinger
...
ENV_NAME="aiG-annotations_hmms-signalp"
...
if ! env_is_complete; then
    ...
    echo ""
    echo "⚠️  REMINDER: SignalP6 itself requires a separate manual install step"
    echo "    (academic license required from DTU). See ai/conda_environment.yml"
    echo "    header for the DOWNLOAD_SOFTWARE-signalp.sh instructions."
fi
...
if [ "${EXECUTION_MODE}" == "slurm" ] && [ -z "${SLURM_JOB_ID}" ]; then
    ...
    SBATCH_ARGS="${SBATCH_ARGS} --output=slurm_logs/signalp-%j.log"
...
# --- Create relative symlinks for per-species consolidated SignalP TSVs ---
# Real files: OUTPUT_pipeline/3-output/{phyloname}_signalp_{mode}_predictions.tsv
# Written by process consolidate_signalp_outputs (script 003). Mode is fast or
# slow per params.mode in nextflow.config; glob covers both.
RESULT_DIR="OUTPUT_pipeline/3-output"
SYMLINK_COUNT=0

for result_file in ${RESULT_DIR}/*_signalp_*_predictions.tsv; do
