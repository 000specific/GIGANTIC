#!/bin/bash
# AI: Claude Code | Opus 4.6 | 2026 March 10 | Purpose: Run TMbed Nextflow pipeline
# Human: Eric Edsinger
...
ENV_NAME="aiG-annotations_hmms-tmbed"
...
if [ "${EXECUTION_MODE}" == "slurm" ] && [ -z "${SLURM_JOB_ID}" ]; then
    ...
    SBATCH_ARGS="${SBATCH_ARGS} --output=slurm_logs/tmbed-%j.log"
...
# --- Create relative symlinks for per-species consolidated TMBed TSVs ---
# Real files: OUTPUT_pipeline/3-output/{phyloname}_tmbed_predictions.tsv
# Written by process consolidate_tmbed_outputs (script 003). The .3line raw
# file lives in 2-output and is consumed by the consolidate step.
RESULT_DIR="OUTPUT_pipeline/3-output"
SYMLINK_COUNT=0

for result_file in ${RESULT_DIR}/*_tmbed_predictions.tsv; do
