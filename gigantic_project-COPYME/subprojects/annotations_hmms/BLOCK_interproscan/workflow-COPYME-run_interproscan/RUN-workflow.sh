...
#   1. BLOCK_interproscan/output_to_input/  (canonical, for downstream subprojects)
...
for result_file in ${RESULT_DIR}/*_interproscan_results.tsv; do
    if [ -f "$result_file" ]; then
        ...
        # BLOCK-level: BLOCK_interproscan/output_to_input/ -> workflow/OUTPUT_pipeline/4-output/
        ln -sf "../${WORKFLOW_DIR_NAME}/${result_file}" "${BLOCK_SHARED_DIR}/${filename}"
        # Workflow archival: ai/output_to_input/ -> ../../OUTPUT_pipeline/4-output/
