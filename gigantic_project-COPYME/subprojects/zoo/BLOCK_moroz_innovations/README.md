...
```
zoo/                                  (non-GIGANTIC holding subproject; see ../README.md)
  ...
  upload_to_server/
    BLOCK_moroz_innovations/          (the structure_001 test-run outputs)
```

This block was formerly the standalone `moroz_innovations` subproject; it was
folded into the `zoo` holding subproject as `BLOCK_moroz_innovations`. Nothing
in the analysis itself changed -- only its location.
