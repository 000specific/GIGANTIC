...
def main() -> int:
    ...
    #
    # Hits whose query OR subject ID cannot be mapped to a row in the gene-
    # coordinates table are dropped. This handles a small but real class of
    # proteome ↔ GFF mismatches (e.g. annotator-flagged "_potentialcontaminant"
    # genes retained in the proteome but dropped from gene_sizes; mitochondrial
    # / hand-curated names in the proteome but absent from the standardized
    # GFF). Audited 2026-05-04 across species70: ~0.16% of total proteome IDs
    # are orphans, concentrated in Pigoraptor spp.
    #
    # User decision (2026-05-04): drop these hits silently from the paralog
    # graph and report the count in the per-species summary. They cannot
    # contribute to a hotspot anyway (a gene without coordinates can't be
    # placed on a chromosome), and the documented patterns make this an
    # explicit, understood drop rather than a silent artifact.
    union_find = UnionFind()
    hits_processed = 0
    hits_in_window = 0
    hits_diff_chromosome = 0
    hits_outside_window = 0
    hits_unmapped_to_coordinates = 0
    ...
    with open( args.filtered_hits, 'r' ) as input_filtered:
        ...
        for line in input_filtered:
            ...
            if query_source_gene_id is None or subject_source_gene_id is None:
                hits_unmapped_to_coordinates += 1
                continue
    ...
    logger.info( f'  Hits unmapped to coordinates:     {hits_unmapped_to_coordinates}  (dropped — gene not in coords table)' )
    logger.info( '' )
    ...
    summary_output += 'Hits_Unmapped_To_Coordinates (dropped because query or subject gene has no row in gene coordinates table)\t'
    ...
    summary_output += args.genus_species + '\t' + str( args.window_size ) + '\t' + str( args.minimum_paralog_count ) + '\t' + str( hits_processed ) + '\t' + str( hits_in_window ) + '\t' + str( hits_diff_chromosome ) + '\t' + str( hits_outside_window ) + '\t' + str( hits_unmapped_to_coordinates ) + '\t' + str( len( hotspot_records ) ) + '\t' + str( member_total ) + '\n'
