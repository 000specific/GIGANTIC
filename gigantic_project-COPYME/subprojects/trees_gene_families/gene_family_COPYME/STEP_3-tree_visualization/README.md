...
- **STEP_2** must be complete for this gene family (tree newicks in `output_to_input/<gene_family>/STEP_2-phylogenetic_analysis/`)
- **Conda** available (env is created automatically on first run)

## Rendering Engine

Uses `toytree` + `toyplot` + `reportlab` (pure Python, no Qt). The conda env `aiG-trees_gene_families-visualization` is created on first run. If the env is broken (e.g., from a prior failed install), `RUN-workflow.sh` detects this and rebuilds it automatically.

## What Gets Rendered

For each gene family, all tree methods that STEP_2 produced:
