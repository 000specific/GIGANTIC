...
**Purpose**: Render gene family phylogenetic trees as PDF + SVG.
...
4. Create symlinks to `output_to_input/<gene_family>/STEP_3-tree_visualization/`
...
Installed via pip inside conda env `aiG-trees_gene_families-visualization`.
...
STEP_3 auto-discovers tree newick files in `output_to_input/<gene_family>/STEP_2-phylogenetic_analysis/` matching these suffixes:
...
| `../../output_to_input/<gene_family>/STEP_3-tree_visualization/` | Rendered PDFs/SVGs | No (auto-created symlinks) |
...
| Memory | 8 GB |
| Time | Seconds to minutes per gene family |
...
|-------|-------|----------|
| "No tree files found" | STEP_2 not complete for this gene family | Run STEP_2 first |
...
find -L ../../../output_to_input/<gene_family>/STEP_2-phylogenetic_analysis/ -name "*.treefile" -o -name "*.fasttree"

# Verify conda env is healthy
conda activate aiG-trees_gene_families-visualization
...
| Situation | Ask |
|-----------|-----|
| Starting STEP_3 | "Has STEP_2 completed for this gene family? Which tree methods ran?" |
