...
Input:
    --orthofinder-dir: Path to OrthoFinder output directory from script 006
    --proteome-list: Path to 1_ai-proteome_list.tsv from script 001

Output:
    OUTPUT_pipeline/7-output/7_ai-orthogroups_gigantic_ids.tsv
        Orthogroups with full GIGANTIC identifiers (standardized format)

    OUTPUT_pipeline/7-output/7_ai-gene_count_gigantic_ids.tsv
        Gene counts per orthogroup per species

Usage:
    python3 007_ai-python-standardize_output.py \\
        --orthofinder-dir OUTPUT_pipeline/6-output \\
...
def setup_logging( output_directory: Path ) -> logging.Logger:
    ...
    # File handler
    log_file = output_directory / '7_ai-log-standardize_output.log'
...
def main():
    ...
    parser.add_argument(
        ...
        type = str,
        required = True,
        help = 'Path to OrthoFinder output directory from script 006'
    )
    ...
    orthogroups_output_path = output_directory / '7_ai-orthogroups_gigantic_ids.tsv'
    orthogroup_count = 0
    total_genes = 0
    ...
    gene_count_output_path = output_directory / '7_ai-gene_count_gigantic_ids.tsv'
