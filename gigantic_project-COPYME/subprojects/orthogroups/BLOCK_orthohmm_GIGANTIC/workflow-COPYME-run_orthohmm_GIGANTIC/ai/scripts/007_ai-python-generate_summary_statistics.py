...
Reads standardized output from script 006.

Input:
    --proteome-list: Path to 1_ai-proteome_list.tsv from script 001
    --orthogroups-file: Path to 6_ai-orthogroups_gigantic_ids.tsv from script 006

Output:
    OUTPUT_pipeline/7-output/7_ai-summary_statistics.tsv
        Overall clustering statistics

    OUTPUT_pipeline/7-output/7_ai-orthogroup_size_distribution.tsv
        Distribution of orthogroup sizes

Usage:
    python3 007_ai-python-generate_summary_statistics.py \\
        --proteome-list OUTPUT_pipeline/1-output/1_ai-proteome_list.tsv \\
        --orthogroups-file OUTPUT_pipeline/6-output/6_ai-orthogroups_gigantic_ids.tsv
"""
...
def setup_logging( output_directory: Path ) -> logging.Logger:
    ...
    # File handler
    log_file = output_directory / '7_ai-log-generate_summary_statistics.log'
...
def main():
    ...
    parser.add_argument(
        ...
        type = str,
        required = True,
        help = 'Path to 6_ai-orthogroups_gigantic_ids.tsv from script 006'
    )

    parser.add_argument(
        '--output-dir',
        type = str,
        default = 'OUTPUT_pipeline/7-output',
        help = 'Output directory (default: OUTPUT_pipeline/7-output)'
    )
    ...
    if not orthogroups_path.exists():
        ...
        logger.error( "Ensure script 006 completed successfully." )
        sys.exit( 1 )
    ...
    summary_file = output_directory / '7_ai-summary_statistics.tsv'
    ...
    size_distribution_file = output_directory / '7_ai-orthogroup_size_distribution.tsv'
