#!/bin/bash
# AI: Claude Code | Opus 4.7 | 2026 April 26 | Purpose: Run BLOCK_orthohmm_GIGANTIC parallel-phmmer NextFlow pipeline
# Human: Eric Edsinger

# =============================================================================
# RUN-workflow.sh
# =============================================================================
# Runs the BLOCK_orthohmm_GIGANTIC NextFlow pipeline.
...
ENV_NAME="ai_gigantic_orthogroups_orthohmm"
...
if ! command -v nextflow &> /dev/null; then
    ...
    if ! command -v nextflow &> /dev/null; then
        ...
        echo "  1. Install nextflow in conda env: conda install -n ai_gigantic_orthogroups_orthohmm -c bioconda nextflow"
...
if [ "${EXECUTION_MODE}" == "slurm" ] && [ -z "${SLURM_JOB_ID}" ]; then
    ...
    SBATCH_ARGS="--job-name=orthohmm_GIGANTIC"
    ...
    SBATCH_ARGS="${SBATCH_ARGS} --output=slurm_logs/orthohmm_GIGANTIC-%j.log"
    ...
    echo "Phmmer fan-out tasks will be submitted by NextFlow as burst-mode SLURM jobs."
...
echo "========================================================================"
echo "Starting BLOCK_orthohmm_GIGANTIC Pipeline"
echo "========================================================================"
...
# Symlinks are created at the subproject-root output_to_input/BLOCK_orthohmm_GIGANTIC/.
#
...
SUBPROJECT_SHARED_DIR="../../output_to_input/BLOCK_orthohmm_GIGANTIC"
...
# Final orthogroup tables (from script 006 — restore_gigantic_identifiers)
ln -sf "../../BLOCK_orthohmm_GIGANTIC/${WORKFLOW_DIR_NAME}/OUTPUT_pipeline/6-output/6_ai-orthogroups_gigantic_ids.tsv" \
    "${SUBPROJECT_SHARED_DIR}/orthogroups_gigantic_ids.tsv"
ln -sf "../../BLOCK_orthohmm_GIGANTIC/${WORKFLOW_DIR_NAME}/OUTPUT_pipeline/6-output/6_ai-gene_count_gigantic_ids.tsv" \
    "${SUBPROJECT_SHARED_DIR}/gene_count_gigantic_ids.tsv"

# Summary statistics (from script 007)
ln -sf "../../BLOCK_orthohmm_GIGANTIC/${WORKFLOW_DIR_NAME}/OUTPUT_pipeline/7-output/7_ai-summary_statistics.tsv" \
    "${SUBPROJECT_SHARED_DIR}/summary_statistics.tsv"

# Per-species QC (from script 008)
ln -sf "../../BLOCK_orthohmm_GIGANTIC/${WORKFLOW_DIR_NAME}/OUTPUT_pipeline/8-output/8_ai-per_species_summary.tsv" \
    "${SUBPROJECT_SHARED_DIR}/per_species_summary.tsv"

echo "  Created symlinks in output_to_input/BLOCK_orthohmm_GIGANTIC/"

echo ""
echo "========================================================================"
echo "SUCCESS! BLOCK_orthohmm_GIGANTIC pipeline complete."
echo ""
echo "Research outputs (real files):"
echo "  OUTPUT_pipeline/1-output/ through 8-output/"
echo ""
echo "Downstream symlinks:"
echo "  ../../output_to_input/BLOCK_orthohmm_GIGANTIC/  (subproject root)"
echo ""
