#!/usr/bin/env python3
# AI: Claude Code | Opus 4.6 | 2026 February 28 | Purpose: Validate proteome directory and create input list for orthogroup workflow
# Human: Eric Edsinger
...
proteome files for the orthogroup detection workflow.

Input:
    --proteomes-dir: Path to gigantic_proteomes directory from genomesDB

Output:
    OUTPUT_pipeline/1-output/1_ai-proteome_list.tsv
...
def setup_logging( output_directory: Path ) -> logging.Logger:
    ...
    # File handler
    log_file = output_directory / '1_ai-log-validate_proteomes.log'
...
def validate_and_list_proteomes( proteomes_directory: Path, output_directory: Path, logger: logging.Logger ) -> None:
    ...
    if not proteomes_directory.exists():
        ...
        logger.error( "Ensure genomesDB has completed and output_to_input is populated." )
        sys.exit( 1 )
    ...
    # Write output file
    output_file = output_directory / '1_ai-proteome_list.tsv'
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description = 'Validate proteome directory and create input list for orthogroup workflow'
    )
    ...
    parser.add_argument(
        ...
        type = str,
        required = True,
        help = 'Path to gigantic_proteomes directory from genomesDB'
    )
