#!/usr/bin/env python3
# AI: Claude Code | Opus 4.5 | 2026 February 27 | Purpose: Convert FASTA headers to short IDs for OrthoHMM
# Human: Eric Edsinger

"""
002_ai-python-convert_headers_to_short_ids.py

Converts GIGANTIC proteome FASTA headers to short sequential IDs for OrthoHMM.
...
Output:
    ...
    OUTPUT_pipeline/2-output/2_ai-header_mapping.tsv
        ...
        - Short_ID (the short header used in OrthoHMM)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description = 'Convert FASTA headers to short IDs for OrthoHMM'
    )
    ...
    with open( mapping_file, 'w' ) as output_mapping:
        # Write header
        header = 'Short_ID (short header for OrthoHMM format Genus_species-N)' + '\t'
