...
# 3. Conda env aiG-orthogroups-orthohmm auto-creates on first run from ai/conda_environment.yml
#
# FOR SLURM CLUSTERS:
...
if [ "${EXECUTION_MODE}" == "slurm" ] && [ -z "${SLURM_JOB_ID}" ]; then
    ...
    SLURM_CPUS=$(read_config "cpus" "32")
    SLURM_MEM=$(read_config "memory_gb" "240")
    ...
    SBATCH_ARGS="${SBATCH_ARGS} --output=slurm_logs/orthohmm-%j.log"
...
# This env is SHARED with BLOCK_orthohmm_GIGANTIC (same OrthoHMM tool).
