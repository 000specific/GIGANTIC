...
Purpose:
    ...
    available genome annotations (GFF/GTF files) from STEP_2 - these are
...
Outputs:
    ...
    1-output/1_ai-species_with_genome_annotations.txt - Species that have GFF/GTF files
...
def get_species_from_genome_annotations( genome_annotations_dir: Path ) -> set:
    """Extract species names from genome annotation filenames (GFF3/GTF)."""
    species_names = set()

    if not genome_annotations_dir or not genome_annotations_dir.exists():
        return species_names
...
def main():
    ...
    # Scan for genome annotations (optional - not all species have them)
    species_with_annotations = []
    if step2_genome_annotations_dir and step2_genome_annotations_dir.exists():
        logger.info( '' )
        logger.info( f'Scanning for genome annotations: {step2_genome_annotations_dir}' )
        all_annotation_species = get_species_from_genome_annotations( step2_genome_annotations_dir )
        logger.info( f'  Found {len( all_annotation_species )} species with genome annotations in STEP_2' )
        ...
        logger.info( f'  {len( species_with_annotations )} of {species_count} selected species have genome annotations' )
        logger.info( f'  {len( species_without_annotations )} selected species lack genome annotations' )

        if species_without_annotations:
            logger.info( '  Species without genome annotations:' )
            for species_name in species_without_annotations:
                logger.info( f'    - {species_name}' )
    else:
        logger.info( '' )
        logger.info( 'No genome annotations directory specified or found - skipping annotation scan' )

    # Write species with genome annotations list
    output_species_with_annotations = output_dir / '1_ai-species_with_genome_annotations.txt'
    with open( output_species_with_annotations, 'w' ) as output_file:
        for species_name in species_with_annotations:
            output = species_name + '\n'
            output_file.write( output )
    logger.info( f'Wrote species with genome annotations list: {output_species_with_annotations} ({len( species_with_annotations )} species)' )

    logger.info( '' )
    logger.info( '=' * 70 )
    logger.info( f'SUCCESS: {species_count} species validated' )
    logger.info( f'  {len( species_with_annotations )} species have genome annotations' )
    logger.info( '=' * 70 )
