#!/bin/bash
# AI: Claude Code | Opus 4.6 | 2026 March 04 | Purpose: Run OCL analysis workflow locally
# Human: Eric Edsinger

################################################################################
# GIGANTIC orthogroups_X_ocl - Origin-Conservation-Loss Analysis (Local)
################################################################################
#
# PURPOSE:
...
# 3. Verify upstream subprojects have completed (trees_species, orthogroups, genomesDB)
#
# COPYME PATTERN:
...
echo "========================================================================"
echo "GIGANTIC OCL Pipeline (Local)"
echo "========================================================================"
echo ""
echo "Started: $(date)"
echo ""
...
if conda activate ai_gigantic_orthogroups_X_ocl 2>/dev/null; then
    echo "Activated conda environment: ai_gigantic_orthogroups_X_ocl"
else
    # Check if nextflow is already available in PATH
    if ! command -v nextflow &> /dev/null; then
        echo "ERROR: Environment 'ai_gigantic_orthogroups_X_ocl' not found!"
        echo ""
...
#   output_to_input/BLOCK_ocl_analysis/Species71_X_OrthoFinder/structure_001/
#   output_to_input/BLOCK_ocl_analysis/Species71_X_OrthoHMM/structure_001/
# ============================================================================
...
for structure_dir in OUTPUT_pipeline/structure_*; do
    if [ -d "$structure_dir" ]; then
        ...
        if [ -f "$summary_file" ]; then
            ...
            ln -sf "../../../BLOCK_ocl_analysis/workflow-COPYME-ocl_analysis/OUTPUT_pipeline/${structure_name}/4-output/4_ai-orthogroups-complete_ocl_summary.tsv" \
...
echo ""
echo "========================================================================"
echo "SUCCESS! OCL pipeline complete."
echo ""
