...
3. Edit START_HERE-user_config.yaml to set:
   - `run_label` for this exploration (e.g., `species70_X_OrthoHMM`)
   - `species_set_name` (e.g., `species70`)
   - `orthogroup_tool` matching your input data (OrthoHMM / OrthoFinder / Broccoli)
   - `orthogroups_dir` pointing to the correct upstream BLOCK for that tool
