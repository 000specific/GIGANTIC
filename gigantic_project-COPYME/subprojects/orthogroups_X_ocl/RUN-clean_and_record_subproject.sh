...
################################################################################
# orthogroups_X_ocl - Clean and Record Subproject
################################################################################
#
# USAGE:
...
if $RECORD; then
    ...
    if [ -f "${PROJECT_ROOT}/RUN-record_project.sh" ]; then
        if $DRY_RUN; then
            echo "  [DRY RUN] Would run: bash ${PROJECT_ROOT}/RUN-record_project.sh --subproject orthogroups_X_ocl"
        else
            bash "${PROJECT_ROOT}/RUN-record_project.sh" --subproject orthogroups_X_ocl
        fi
    else
