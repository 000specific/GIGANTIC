# AI: Claude Code | Opus 4.8 (1M context) | 2026 June 28 | Purpose: annogroups source parser for SUPERFAMILY (per-species InterProScan-parsed superfamily files)
# Human: Eric Edsinger

"""
annogroups source parser — superfamily.

Reads the per-species SUPERFAMILY annotation files exposed by annotations_hmms at
  annotations_hmms/output_to_input/BLOCK_interproscan_parsed/superfamily/superfamily-<phyloname>.tsv
and returns, per sequence, the list of SUPERFAMILY features (all positional).

SUPERFAMILY SCOP structural-domain HMM models (SSF accessions). Each match carries residue coordinates (Match_Start / Match_End), so
superfamily participates in all four annogroup types (feature, combination,
architecture, absent) -- combination groups by the whole-protein set of SUPERFAMILY
accessions, architecture by their sub-protein N->C arrangement.

The parsed superfamily files share the identical 15-column self-documenting schema as
the pfam files, so this parser mirrors parsers/pfam.py (only SOURCE and the source
subpath differ). Example accession: SSF48726.
...
RELATIVE_SUBPATH = "BLOCK_interproscan_parsed/superfamily"
...
def parse_source_features( workflow_root: Path, config: dict ) -> dict:
    ...
    if not annotations_dir.is_dir():
        print( f"CRITICAL ERROR: superfamily annotation directory not found: {annotations_dir}", file = sys.stderr )
        print( "  Verify annotations_hmms exposed BLOCK_interproscan_parsed/superfamily/ in output_to_input.", file = sys.stderr )
        sys.exit( 1 )

    source_files = sorted( annotations_dir.glob( "superfamily-*.tsv" ) )
    if not source_files:
        print( f"CRITICAL ERROR: no superfamily-*.tsv files in {annotations_dir}", file = sys.stderr )
        sys.exit( 1 )
...
def parse_source_definitions( workflow_root: Path, config: dict ) -> dict:
    """
    accession -> human-readable definition (SUPERFAMILY signature description, column
