# AI: Claude Code | Opus 4.8 (1M context) | 2026 June 28 | Purpose: annogroups source parser for SMART (per-species InterProScan-parsed smart files)
# Human: Eric Edsinger

"""
annogroups source parser — smart.

Reads the per-species SMART annotation files exposed by annotations_hmms at
  annotations_hmms/output_to_input/BLOCK_interproscan_parsed/smart/smart-<phyloname>.tsv
and returns, per sequence, the list of SMART features (all positional).

SMART domain signatures (SM accessions). Each match carries residue coordinates (Match_Start / Match_End), so
smart participates in all four annogroup types (feature, combination,
architecture, absent) -- combination groups by the whole-protein set of SMART
accessions, architecture by their sub-protein N->C arrangement.

The parsed smart files share the identical 15-column self-documenting schema as
the pfam files, so this parser mirrors parsers/pfam.py (only SOURCE and the source
subpath differ). Example accession: SM00409.
...
RELATIVE_SUBPATH = "BLOCK_interproscan_parsed/smart"
...
def parse_source_features( workflow_root: Path, config: dict ) -> dict:
    ...
    if not annotations_dir.is_dir():
        print( f"CRITICAL ERROR: smart annotation directory not found: {annotations_dir}", file = sys.stderr )
        print( "  Verify annotations_hmms exposed BLOCK_interproscan_parsed/smart/ in output_to_input.", file = sys.stderr )
        sys.exit( 1 )

    source_files = sorted( annotations_dir.glob( "smart-*.tsv" ) )
    if not source_files:
        print( f"CRITICAL ERROR: no smart-*.tsv files in {annotations_dir}", file = sys.stderr )
        sys.exit( 1 )
...
def parse_source_definitions( workflow_root: Path, config: dict ) -> dict:
    """
    accession -> human-readable definition (SMART signature description, column
